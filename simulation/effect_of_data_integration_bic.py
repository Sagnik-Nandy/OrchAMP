...
import numpy as np
import pandas as pd
import complete_pipeline_bic
importlib.reload(complete_pipeline_bic)
MultimodalPCAPipeline = complete_pipeline_bic.MultimodalPCAPipeline
SeparatePCAPipeline = complete_pipeline_bic.SeparatePCAPipeline
...
def generate_tri_modal(
    ...
    Key constructs:
      U1_i = sqrt(2/3) * (T1_i + T2_i),  T1_i ~ Unif(S^{r1-1}),  T2_i ~ Unif({Â±1}^{r1})
      U2_Â·1 = Ï * stdz(truncate-linear(U1_Â·1)) + sqrt(1-Ï^2) * Z,  Z ~ N(0,1)
      U2_Â·2 = Ï * stdz(max(0, U1_Â·2)) + sqrt(1-Ï^2) * B,         B ~ Rademacher(Â±1)
      U2_Â·3 ~ Laplace(0, 1/â2)  (extra cols, if any, are independent Laplace)
      U3_i = ÏÌ * U1_i + sqrt(1-ÏÌ^2) * T3_i,  T3_i ~ Unif({Â±1}^{r1})
      Rows of V_h are i.i.d. â(r_h) * Unif(S^{r_h-1})
    ...
    T2 = np.random.choice([-1, 1], size=(N, r1))               # Unif on {Â±1}^{r1}
    ...
    # -----------------------
    # Loadings V1, V2 : rows ~ â(r_h) * Unif(S^{r_h-1})
    # -----------------------
...
def run_data_integration(N, lam, rho, gamma1, gamma2, reps=50):
    ...
    """
    errors = np.zeros((reps, 12), dtype=float)  # 3 methods Ã 3 modalities + 3 for U2 first two cols
    ...
    for i in range(reps):
        ...
        pipeline = MultimodalPCAPipeline(
            n_components_u=max(1, int(np.cbrt(X1.shape[0]))),
            n_components_v_list=[max(1, int(np.cbrt(X1.shape[1]))), max(1, int(np.cbrt(X2.shape[1])))]
        )
        ...
        sep_pipeline = SeparatePCAPipeline(
            n_components_list_v=[max(1, int(np.cbrt(X1.shape[1]))), max(1, int(np.cbrt(X2.shape[1])))],
            n_components_list_u=[max(1, int(np.cbrt(X1.shape[0]))), max(1, int(np.cbrt(X2.shape[0]))), max(1, int(np.cbrt(X3.shape[0])))]
        )
...
def main():
    ...
    # ===== save results =====
    out_dir = "/home/nandy.15/Research/Experiments_revision/Results/Effect_of_data_integration_bic"
    ...
    #    X1 (modality 1): columns 0,3,6 â joint/sep/svd for U1
    df_X1 = pd.DataFrame({
        "setting": [label]*reps,
        "lambda":  [lam]*reps,
        "rho":     [rho]*reps,
    ...
    #    X2 (modality 2): columns 1,4,7 â joint/sep/svd for U2
    df_X2 = pd.DataFrame({
        "setting": [label]*reps,
        "lambda":  [lam]*reps,
        "rho":     [rho]*reps,
    ...
    #    X3 (modality 3): columns 2,5,8 â joint/sep/svd for U3
    df_X3 = pd.DataFrame({
        "setting": [label]*reps,
        "lambda":  [lam]*reps,
        "rho":     [rho]*reps,
