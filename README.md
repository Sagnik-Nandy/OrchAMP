...
### Data source
We provide the pre-processed CITE-seq dataset from [Hao et al. (2021)](https://www.sciencedirect.com/science/article/pii/S0092867421005833) [here](https://www.dropbox.com/home/PhD%20Projects/Current_Projects_with_Seniors/AMP%2BMultimodal_Data/Data_CITE_seq_GITHUB). We have subsampled 10,000 cells randomly from the database after pre-processing using the standard steps described in [Hao et al. (2021)](https://www.sciencedirect.com/science/article/pii/S0092867421005833). (**Please download the data from the provided url. The raw data cannot be uploaded due to size constarints.**)

## Multimodal Atlas Building
Using these datasets, one can reproduce Figure 6 in the supplement of our paper by running `atlas_building_whole_pbmc_data_gmm.ipynb` in the `cite_seq` folder. We have benchmarked against `WNN` (PCA computed using the same jupyter notebook), MOFA+ (embeddings computed by running `cite_seq_mofa.R`) and [totalVI](https://pubmed.ncbi.nlm.nih.gov/33589839/) (embeddings computed by running `atlas_building_whole_pbmc_data_total_vi_new.ipynb`). To run `totalVI`, we may need to download specific packages used to run `scVI` (See, [documentation](https://scvi-tools.org/)). The UMAP plots are generated by running `plot_for_cite_seq.R`.
